...
const selection = (list: number[], stepsLog:stepsLogT) => {
  ...
  for (let i = 0; i < output.length-1; i++) {
    ...
    for (let j = i + 1; j < output.length; j++) {
      stepsLog.push({type: 'compare', elements: [{type: 'index', value: minSelected}, {type: 'index', value: j}]});
    ...
    stepsLog.push({type: 'swap', elements: [{type: 'index', value: minSelected}, {type: 'index', value: i}]});
    if (minSelected != i) {
